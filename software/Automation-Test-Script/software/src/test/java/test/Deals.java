	...
	public class Deals {
		...
		public void beforeClass()throws InterruptedException, IOException
			{
				report = new ExtentReports("VerificationOfDeals" + sd.format(date) + ".html", false);
		...
		@DataProvider(name="ICnames")
		 public Object[] data()
		        {
		  return new Object [] {"Technology","Foodservice","Construction","Consumer","Apparel","Mining","Insurance","Retail","Automotive","Disruptor","Packaging","Travel & Tourism","Aerospace, Defense & Security","Banking & Payments","Explorer"};
		         }
	  ...
	  
	  {
			...
			if(icName.equals("Banking & Payments"))
			 {
			...
			if(driver.findElements(By.xpath("//a[@id='dealslisting']")).size()>0)
			 {
				try
				{
						...
						test.log(LogStatus.PASS, "All Deals page loaded successfully");
					
				}
				catch(Exception co)
				{
					test.log(LogStatus.FAIL, "All Deals  page content failed to  load ");
					test.log(LogStatus.INFO, co.getMessage());
					test.log(LogStatus.FAIL,test.addScreenCapture(capture(driver))+ "All Deals page content failed to  load ");	
				}
			 }
			 else
			 {
				test.log(LogStatus.PASS, "All Deals page not available");
	...
	if(driver.findElements(By.xpath("//a[@id='rCompanies']")).size()>0)
	 {
		try
		{
				...
				js.executeScript(";arguments[0].click();", detailedpage);
				w.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[text()=' Target Companies related to deal results']")));
	...
	
	{
		...
		if(driver.findElements(By.xpath("(//a[@target='_blank'])[3]")).size()>0)
		 {
			try
			{
					WebElement detailedpage= driver.findElement(By.xpath("(//a[@target='_blank'])[3]"));
					js.executeScript(";arguments[0].click();", detailedpage);
					...
					driver.switchTo().window(child);
					w.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[@class='regular']")));
					test.log(LogStatus.PASS, "Disruptive Tech deal detail page loaded successfully");
					driver.close();
			...
			catch(Exception co)
			{
				test.log(LogStatus.FAIL, "Deal detail page  content failed to  load ");
				test.log(LogStatus.INFO, co.getMessage());
				test.log(LogStatus.FAIL,test.addScreenCapture(capture(driver))+ "Disruptive page content failed to  load ");	
			}
		 }
		else
		{
			test.log(LogStatus.PASS, "Disruptive Tech deal detail page not available");
